...
class UserLoginViewController: UIViewController
{
    ...
    override func viewDidLoad()
    {
        ...
        imgUser.image = UIImage(named: "iconfont-userme")
    ...
    @objc func loginEvent()
    {
        ...
        if usercode == "2018302060038" && password == "123"
        {
            defaults.set(usercode, forKey: stuNum)
            defaults.set(true, forKey: hasLogined)
        ...
        else
        {
            ...
            p.addAction(UIAlertAction(title: "确定", style: .default, handler:
                                        {(_:UIAlertAction) in self.txtPwd.text = ""}))
